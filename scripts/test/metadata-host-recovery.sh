...
cargo test -p multiplex-store --test metadata_restore_crash_matrix
cargo test -p multiplex-session-host --test lease_reconciliation
cargo test -p multiplex-session-host --test no_unproven_signal
cargo test -p multiplex -- ui::recovery

if rg -n 'libc::kill|std::process|Command::new|process\.kill|factory.reset|remove_dir_all' \
  crates/multiplex-store/src/recovery.rs \
  crates/multiplex-client/src/host_recovery.rs; then
...
for selector in \
  ...
  rg -q "\"$selector\"" crates/multiplex-desktop/src/ui/app/session_sidebar.rs || {
