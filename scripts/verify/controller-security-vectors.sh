...
if rg -n 'TcpListener|UdpSocket|std::net|tokio|uniffi' crates/multiplex-controller-security/src; then
  echo "controller-security crate contains a forbidden transport or FFI surface" >&2
  exit 1
fi

cargo test -p multiplex-controller-security --test golden_vectors --locked
