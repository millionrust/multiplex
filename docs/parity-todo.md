...
- macOS: cargo-bundle metadata is in place (`[package.metadata.bundle]` in crates/termirust-desktop/Cargo.toml). `cargo bundle --release` produces `Multiplex.app`. Signed/notarized distribution requires an Apple Developer Program membership; flow documented in docs/building.md.
