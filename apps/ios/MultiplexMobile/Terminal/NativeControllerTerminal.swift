...
final class NativeControllerTerminalSession: @unchecked Sendable {
    ...
    init?(
        ...
        handle = multiplex_mobile_terminal_create(
    ...
    func feed(_ data: Data) throws {
        ...
        let accepted = data.withUnsafeBytes { bytes in
            multiplex_mobile_terminal_feed(
                handle,
                bytes.bindMemory(to: UInt8.self).baseAddress,
                data.count
            )
    ...
    func resize(_ viewport: TerminalViewportState) throws -> BoundedTerminalSnapshot {
        guard let handle else { throw NativeTerminalError.closed }
        return try decode(multiplex_mobile_terminal_resize(
            handle,
            UInt16(clamping: viewport.columns),
            UInt16(clamping: viewport.rows)
        ))
    ...
    func snapshot() throws -> BoundedTerminalSnapshot {
        guard let handle else { throw NativeTerminalError.closed }
        return try decode(multiplex_mobile_terminal_snapshot(handle))
    }

    private func decode(_ result: MultiplexMobileResult) throws -> BoundedTerminalSnapshot {
        defer { multiplex_mobile_free_result(result) }
    ...
    private func data(from buffer: MultiplexMobileByteBuffer) -> Data {
