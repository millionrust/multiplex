...
fn verification_reports_old_and_broken_tmux() {
    ...
    assert_eq!(
        ...
        Err(multiplex_tmux::VerificationError::TooOld)
    );
    let other = tempfile::tempdir().unwrap();
...
exit 0"#,
    ...
    assert_eq!(
        ...
        Err(multiplex_tmux::VerificationError::NotListed)
    );
}
...
fn shell_integration_starts_new_terminal_app_shells_inside_tmux() {
    use multiplex_tmux::shell_integration::{Shell, ShellIntegration};
...
fn the_setup_hides_wrapped_status_bars_and_clears_the_old_scrollback_override() {
    ...
    server.run(&[
        "set-option",
        "-s",
        multiplex_tmux::LEGACY_SCROLLBACK_OVERRIDE_TARGET,
        "*:smcup@:rmcup@",
    ]);
    ...
    assert_eq!(
        ...
        multiplex_tmux::appearance::SELECTION_STYLE
    );
...
fn a_file_dropped_on_a_scrolled_back_tab_reaches_the_program() {
    ...
    let version = multiplex_tmux::parse_version(server.tmux.version());
...
fn a_wrapped_session_differs_from_a_plain_one_only_where_intended() {
    ...
    fs::write(
        &config,
        multiplex_tmux::appearance::WrappedSessionAppearance::for_this_platform()
            .configuration_file(),
    )
    .unwrap();
...
fn a_session_sourcing_the_generated_configuration_gets_it_and_others_do_not() {
    ...
    fs::write(
        &config,
        multiplex_tmux::appearance::WrappedSessionAppearance::new(Some("pbcopy".to_owned()))
            .configuration_file(),
    )
    .unwrap();
    server.run(&["new-session", "-d", "-s", "work"]);
