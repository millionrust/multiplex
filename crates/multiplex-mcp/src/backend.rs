...
impl LocalInspectionSource {
    ...
    fn ingest_artifact_bytes(
        ...
        if content.is_empty()
            || content.len() as u64 > multiplex_domain::MAX_ARTIFACT_BYTES
...
fn map_cli_error(error: multiplex_cli::CliError) -> SourceError {
    use multiplex_cli::ErrorCode;
    match error.code {
...
fn map_artifact_error(error: multiplex_store::ArtifactStoreError) -> SourceError {
    use multiplex_store::ArtifactStoreError;
    match error {
        ArtifactStoreError::Domain(multiplex_domain::ArtifactError::Cancelled) => {
            SourceError::Cancelled
        }
        ArtifactStoreError::Domain(multiplex_domain::ArtifactError::PermissionDenied) => {
            SourceError::PermissionDenied
        }
        ArtifactStoreError::Domain(
            multiplex_domain::ArtifactError::ItemQuotaExceeded
            | multiplex_domain::ArtifactError::SessionQuotaExceeded
            | multiplex_domain::ArtifactError::GlobalQuotaExceeded
            | multiplex_domain::ArtifactError::CountQuotaExceeded,
        )
