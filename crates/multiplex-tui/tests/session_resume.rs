...
#[path = "../../multiplex-cli/tests/common/mod.rs"]
mod cli_common;
...
use multiplex_session_host::{LaunchDescriptor, start};
use multiplex_store::{ContinuityRepository, HostLease, HostMetadata, PresetRepository};
use multiplex_tui::{LocalResumeExecutor, ResumeExecutor};
...
async fn tui_resume_real_successor_host_survives_executor_return() {
    ...
    assert_eq!(
        state.lifecycle,
        multiplex_host_protocol::wire::Lifecycle::Ready as i32
    );
