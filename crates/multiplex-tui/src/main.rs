...
use multiplex_tui::source::LocalFleetSource;
use multiplex_tui::terminal::{RunOptions, run};
...
fn main() {
    match parse_options(std::env::args().skip(1)) {
        ...
        Err(error) => {
            eprintln!("multiplex-tui: {error}");
            eprintln!("Run multiplex-tui --help for usage.");
            std::process::exit(2);
        }
    }
}
