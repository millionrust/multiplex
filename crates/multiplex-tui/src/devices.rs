...
use multiplex_domain::{ControllerDeviceId, DeviceStoreRevision};
...
impl LocalDeviceExecutor {
    pub fn new(config_root: PathBuf) -> Self {
        Self {
            paths: CliPaths::new(config_root, PathBuf::from("multiplex-session-host")),
        }
    }
...
fn map_cli_error(error: multiplex_cli::CliError) -> DeviceFailure {
    DeviceFailure {
