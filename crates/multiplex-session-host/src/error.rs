...
impl From<multiplex_host_protocol::CodecError> for HostError {
    fn from(error: multiplex_host_protocol::CodecError) -> Self {
        match error {
            multiplex_host_protocol::CodecError::FrameTooLarge => {
...
impl From<multiplex_store::JournalError> for HostError {
    fn from(_: multiplex_store::JournalError) -> Self {
        Self::new(HostErrorCode::Journal)
    }
}

impl From<multiplex_store::LeaseError> for HostError {
    fn from(error: multiplex_store::LeaseError) -> Self {
        match error.code {
            multiplex_store::LeaseErrorCode::Busy => Self::new(HostErrorCode::LeaseBusy),
            multiplex_store::LeaseErrorCode::PermissionDenied
            | multiplex_store::LeaseErrorCode::UnsafeEntry => {
