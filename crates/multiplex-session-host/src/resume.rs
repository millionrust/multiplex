...
fn build_codex_resume_plan_with_limits(
    ...
    if arguments.len() > multiplex_domain::MAX_RESUME_ARGUMENTS
        || arguments.iter().any(|argument| {
            argument.len() > multiplex_domain::MAX_RESUME_ARGUMENT_BYTES || argument.contains('\0')
        })
    {
    ...
    fn candidate(fingerprint: ExecutableFingerprint) -> ResumeCandidate {
        ResumeCandidate {
            request: ResumeRequest {
                ...
                expected_revision: multiplex_domain::Revision::new(1),
            },
