...
use multiplex_host_protocol::wire::{self, envelope_payload};
use multiplex_host_protocol::{
...
async fn observe_quiet_prompt(state: &RuntimeState) -> Result<(), HostError> {
    ...
    if quiet_nanos < multiplex_domain::HEURISTIC_IDLE_QUIET_NANOS {
        return Ok(());
    }
...
fn wire_activity(activity: multiplex_domain::ActivityState) -> wire::Activity {
    match activity {
        multiplex_domain::ActivityState::Unknown => wire::Activity::Unknown,
        multiplex_domain::ActivityState::Idle => wire::Activity::Idle,
        multiplex_domain::ActivityState::Busy => wire::Activity::Busy,
        multiplex_domain::ActivityState::NeedsInput => wire::Activity::NeedsInput,
        multiplex_domain::ActivityState::Done => wire::Activity::Done,
        multiplex_domain::ActivityState::Failed => wire::Activity::Failed,
    }
}
...
fn wire_attention_reason(
    reason: Option<multiplex_domain::AttentionReason>,
) -> wire::AttentionReason {
    match reason {
        None => wire::AttentionReason::Unspecified,
        Some(multiplex_domain::AttentionReason::Input) => wire::AttentionReason::Input,
        Some(multiplex_domain::AttentionReason::Approval) => wire::AttentionReason::Approval,
        Some(multiplex_domain::AttentionReason::Permission) => wire::AttentionReason::Permission,
        Some(multiplex_domain::AttentionReason::Confirmation) => {
...
async fn serve_attach(
    ...
    let read = match initial_read {
        Ok(read) => read,
        Err(error) if error.code == multiplex_store::JournalErrorCode::HistoryUnavailable => {
...
mod tests {
    ...
    fn descriptor_with_detection(
        executable: PathBuf,
        fingerprint: multiplex_domain::ExecutableFingerprint,
    ) -> LaunchDescriptor {
