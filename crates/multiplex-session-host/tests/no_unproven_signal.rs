...
use multiplex_domain::{ActivityAggregate, HostInstanceId, HostLifecycle, HostedSessionId};
use multiplex_host_protocol::opaque_endpoint_name;
use multiplex_store::{HostLease, HostMetadata, RecoveryResult};
...
fn recovery_module_has_no_process_control_surface() {
    let source = include_str!("../../multiplex-client/src/host_recovery.rs");
    for forbidden in [
        "libc::kill",
        "std::process",
        "Command::new",
