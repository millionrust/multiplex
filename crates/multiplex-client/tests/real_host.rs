...
use multiplex_client::{ClientErrorCode, ConnectOptions, HostClient, LocalEndpoint};
...
use multiplex_store::{HostLease, JournalLimits, JournalStore, read_host_metadata};
...
async fn real_host_compacts_to_snapshot_and_quota_pause_keeps_pty_alive() {
    ...
    while Instant::now() < deadline {
        ...
        if stats.recording_paused {
            assert_eq!(stats.lifecycle, multiplex_domain::HostLifecycle::Ready);
