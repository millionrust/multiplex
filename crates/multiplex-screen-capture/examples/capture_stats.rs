...
//!
//! `cargo run -p multiplex-screen-capture --release --example capture_stats -- 10 2.0`
//!
...
fn main() {
    ...
    use multiplex_screen_capture::{CaptureConfig, FrameSource, displays};
    use multiplex_screen_codec::{
    ...
    config.max_fps = 30;
    #[cfg(target_os = "macos")]
    let started_source = multiplex_screen_capture::ScreenCaptureKitSource::start(config);
    #[cfg(target_os = "windows")]
    let started_source = multiplex_screen_capture::DesktopDuplicationSource::start(config);
