...
use multiplex_store::replication::MAX_REPLICATION_REPOSITORY_BYTES;
use multiplex_store::{
...
fn coordinator(
    ...
    shared_root: &Path,
    workspace: multiplex_domain::ReplicationWorkspaceId,
...
fn resolutions_for(
    plan: &multiplex_store::ReplicationSyncPlan,
...
fn two_devices_diverge_resolve_restart_and_converge_without_reviving_revoked_authority() {
    ...
    assert!(matches!(
        ...
        Err(ReplicationStoreError::Domain(
            multiplex_domain::ReplicationError::ReplicaNotActive
        ))
    ));
}
...
fn recovery_refuses_newer_wrong_workspace_and_existing_or_unsafe_evidence() {
    ...
    let wrong_workspace = multiplex_domain::ReplicationWorkspaceId::new("workspace-other").unwrap();
