...
fn unused_enrollment_intent_requires_the_original_accessible_identity() {
    use multiplex_replication_security::{ReplicationKeyEpoch, ReplicationSecretVault};
    ...
    assert!(
        member
            .review_record_transfer(
                &vec![0; multiplex_domain::MAX_REPLICATION_DOCUMENT_BYTES + 1],
                &selected
            )
            .is_err()
    );
...
fn enrollment_rotation_revocation_and_outbox_ack_survive_restart() {
    ...
    let mut service =
        ReplicationProductService::bootstrap(&root, shared.path(), backend.clone()).unwrap();
    let second_replica = multiplex_domain::ReplicationReplicaId::new("device-phone").unwrap();
...
fn real_two_device_enrollment_converges_with_member_only_custody() {
    ...
    let rotation = multiplex_store::ReplicationAuthorityUpdate::from_canonical_bytes(
...
fn local_replica_deletion_requires_confirmation_and_rejects_stale_plan() {
    ...
    assert!(matches!(
        service.delete_local_replica(
            &stale,
            multiplex_store::ReplicationDeletionPlan::confirmation_phrase(),
        ),
