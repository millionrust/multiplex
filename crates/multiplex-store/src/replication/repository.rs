...
use multiplex_domain::{ReplicationDocument, ReplicationPolicy, ReplicationWorkspaceId};
use multiplex_replication_security::{ReplicationSecretBackend, ReplicationSecretRef};
...
impl ReplicationRepository {
    ...
    pub fn retire_pending<B: ReplicationSecretBackend>(
        ...
        for reference in &references {
            match backend
                .delete(reference)
                .map_err(multiplex_replication_security::ReplicationSecretCustodyError::from)?
            {
                true => deleted += 1,
...
fn validate_retirements(
    ...
    if references.len() > multiplex_replication_security::MAX_REPLICATION_RETAINED_EPOCH_KEYS {
        return Err(ReplicationStoreError::TooManyRetirements);
    }
    let mut unique = std::collections::BTreeSet::new();
    for reference in references {
        if reference.kind() != multiplex_replication_security::ReplicationSecretKind::EpochKey {
