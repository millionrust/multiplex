...
//!
//! `cargo run -p multiplex-screen-host --release --example motion_report`
//!
//! The tile path's workloads are measured in `multiplex-screen-codec`'s `workload_report`. This
...
use multiplex_screen_host::{Ladder, MotionSender, Rung, platform_encoders};
use multiplex_screen_protocol::{
                ...
                if warming {
                    ...
                    match &message {
                        Message::Batch(batch) => {
                            ...
                            for op in &batch.ops {
                                let tile = match op {
                                    multiplex_screen_codec::TileOp::Solid { tile, .. }
                                    | multiplex_screen_codec::TileOp::Cached { tile, .. }
                                    | multiplex_screen_codec::TileOp::Lossy { tile, .. }
                                    | multiplex_screen_codec::TileOp::Lossless { tile, .. } => {
                                        Some(*tile)
                                    }
                                    _ => None,
                                };
