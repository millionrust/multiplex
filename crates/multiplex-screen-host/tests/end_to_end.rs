...
use multiplex_screen_session::{HostConfig, InputEvent, ViewerSession};
...
impl ControllerConnectionBackend for NoTerminals {
    async fn command_context(
        &mut self,
        _: &multiplex_controller_listener::ControllerCommandEnvelope,
        _: &CancellationToken,
    ...
    async fn execute(
        &mut self,
        command: multiplex_controller_listener::ControllerCommandEnvelope,
        _: &CancellationToken,
    ...
    async fn send(&mut self) {
        while let Some(message) = self.session.poll_outgoing() {
            let capability = match message.input_kind() {
                ...
                Some(multiplex_screen_protocol::InputKind::Pointer) => {
                    ScreenFrameCapability::Pointer
                }
                Some(multiplex_screen_protocol::InputKind::Keyboard) => {
