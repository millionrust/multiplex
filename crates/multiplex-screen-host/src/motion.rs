...
//! streaming (`multiplex-screen-codec`'s motion tracker) and tells the viewer about it. This
...
//! real one is `multiplex-screen-video`'s VideoToolbox session; the tests here use a fake.
//!
...
use multiplex_screen_codec::{Frame, REGION_COST_WINDOW_MS, Rect};
use multiplex_screen_protocol::{FeatureSet, MotionCodec, VideoConfig, VideoFrame};
use multiplex_screen_session::{HostSession, TicketVerifier};
