...
impl Gallery {
    fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        tree.split(&1, 2, multiplex_slate::DropEdge::Right, 4);
        tree.split(&2, 3, multiplex_slate::DropEdge::Bottom, 4);
    ...
    fn split_focused(&mut self, edge: multiplex_slate::DropEdge, cx: &mut Context<Self>) {
        ...
        let header = WorkspaceHeader::new(name)
            ...
            .tool(
                ToolbarButton::new("ws-split-right")
                    ...
                    .on_click(cx.listener(|this, _, _, cx| {
                        this.split_focused(multiplex_slate::DropEdge::Right, cx)
                    })),
            )
            .tool(
                ToolbarButton::new("ws-split-down")
                    ...
                    .icon(IconName::SplitDown)
                    .on_click(cx.listener(|this, _, _, cx| {
                        this.split_focused(multiplex_slate::DropEdge::Bottom, cx)
                    })),
            );
...
fn main() {
    application()
        ...
        .run(|cx: &mut App| {
            multiplex_slate::init(theme_from_env(), cx);
            cx.bind_keys([
