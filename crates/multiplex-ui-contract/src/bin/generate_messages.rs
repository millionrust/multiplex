...
use multiplex_ui_contract::{generate_message_artifacts, load_catalog, load_message_schema};
...
fn run() -> Result<(), String> {
    ...
    let rust_path = root.join("crates/multiplex-ui-contract/src/generated_messages.rs");
...
fn check_file(path: &Path, expected: &str) -> Result<(), String> {
    ...
    if actual == expected {
        ...
        Err(format!(
            "{} is stale; run `cargo run -p multiplex-ui-contract --bin generate-messages`",
            path.display()
        ))
    }
}
