...
use common::*;
use multiplex_cli::{Cancellation, ErrorCode, LocalCommandService, RunOutput, run};
use multiplex_domain::{HostFingerprint, HostPublicKey, PairedDeviceStatus};
...
fn packaged_cli_reads_and_revokes_the_desktop_device_authority() {
    let seed = seed_store();
    let initial = seed_controller_devices(&seed);
    let binary = env!("CARGO_BIN_EXE_multiplex-cli");
