...
use common::*;
use multiplex_cli::{Cancellation, CliCommand, CliData, CommandService, ErrorCode};
...
fn pre_ready_cancellation_cleans_host_and_post_ready_cancellation_detaches_successfully() {
    ...
    let launcher = Arc::new(Mutex::new(FakeLauncherState {
        ...
        descriptors: Vec::new(),
        outcome: Some(Ok(
            multiplex_cli::HostLaunchOutcome::ReadyAfterPreReadyCancellation,
        )),
    ...
    let launcher = Arc::new(Mutex::new(FakeLauncherState {
        calls: 0,
        descriptors: Vec::new(),
        outcome: Some(Ok(multiplex_cli::HostLaunchOutcome::Ready)),
