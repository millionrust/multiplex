...
impl StoredControllerProfile {
    ...
    fn from_pairing(
        ...
        result: &multiplex_controller_listener::ControllerClientPairingResult,
    ) -> Self {
        Self {
...
impl SystemSshControllerExecutor {
    ...
    fn confirm_pairing(
        &self,
        sas: &multiplex_controller_security::SasCode,
    ...
    pub fn execute_interactive_attach(
        ...
        if !matches!(command.action, ControllerSshAction::Attach { .. }) {
            return Err(CliError::new(
                ...
                "Run multiplex-cli controller ssh ... attach with a session and generation.",
            ));
        }
...
async fn execute_pair_route<F, G>(
    ...
    cancellation: &Cancellation,
) -> Result<multiplex_controller_listener::ControllerClientPairingResult, CliError>
where
    F: FnOnce(&multiplex_controller_security::SasCode) -> Result<bool, ListenerError>,
    G: FnOnce(
        &multiplex_controller_listener::ControllerClientPairingResult,
...
async fn wait_for_interactive_response(
    channel: &mut ControllerClientChannel<ChildDuplex>,
    expected_id: multiplex_domain::CommandId,
    expected: InteractiveExpected,
    session_id: HostedSessionId,
    last_sequence: &mut multiplex_domain::OutputSequence,
...
fn write_live_output(
    stdout: &mut impl Write,
    last_sequence: &mut multiplex_domain::OutputSequence,
    sequence: multiplex_domain::OutputSequence,
    bytes: &[u8],
) -> Result<(), CliError> {
...
async fn attach(
    ...
    session_id: HostedSessionId,
    occupant_generation: OccupantGeneration,
    from_sequence: multiplex_domain::OutputSequence,
    columns: u16,
    rows: u16,
) -> Result<(), CliError> {
    let command_id = channel
...
fn map_spawn(error: multiplex_client::SshControllerError) -> CliError {
    match error.code {
...
mod tests {
    use super::*;
    use multiplex_client::{SshControllerTargetId, ValidatedDnsOrIp, ValidatedUser};
    ...
    fn public_pairing_profile_is_atomic_bounded_and_contains_no_private_key() {
        ...
        let result = multiplex_controller_listener::ControllerClientPairingResult {
    ...
    fn interactive_output_drops_duplicates_and_fails_on_gaps() {
        ...
        let mut sequence = multiplex_domain::OutputSequence::new(4);
        write_live_output(
            &mut output,
            &mut sequence,
            multiplex_domain::OutputSequence::new(5),
            b"five",
        )
        .unwrap();
        write_live_output(
        ...
        assert_eq!(
            write_live_output(
                &mut output,
                &mut sequence,
                multiplex_domain::OutputSequence::new(7),
                b"gap",
            )
            .unwrap_err()
            .code,
