...
use multiplex_client::{ClientError, ClientErrorCode, ConnectOptions, HostClient, LocalEndpoint};
...
use multiplex_host_protocol::{CURRENT_PROTOCOL, wire};
...
pub fn cli_installation_status(current_executable: &Path) -> CliInstallationStatus {
    let path = sibling_binary(current_executable, "multiplex-cli");
    let host_path = sibling_binary(current_executable, "multiplex-session-host");
...
pub struct HostAttachSummary {
    pub lifecycle: multiplex_domain::HostLifecycle,
...
impl LocalCommandService {
    ...
    fn session_input(
        ...
        if metadata.session_id != session_id
            || metadata.lifecycle != multiplex_domain::HostLifecycle::Ready
        {
    ...
    fn resume_launch_descriptor(
        &self,
        plan: &ResumePlan,
        replacement_generation: multiplex_domain::OccupantGeneration,
    ) -> LaunchDescriptor {
    ...
    pub(crate) fn validate_session_attach(
        ...
        if matches!(
            metadata.lifecycle,
            multiplex_domain::HostLifecycle::Starting | multiplex_domain::HostLifecycle::Orphaned
        ) {
            return Err(unavailable(
    ...
    fn launch_descriptor(&self, resolved: &multiplex_domain::ResolvedLaunch) -> LaunchDescriptor {
        LaunchDescriptor {
            format_version: LaunchDescriptor::FORMAT_VERSION,
...
) -> Result<&multiplex_domain::PairedDeviceRecord, CliError> {
    snapshot
        .authority
        .devices
        .iter()
...
fn require_device_not_revoked(
    device: &multiplex_domain::PairedDeviceRecord,
) -> Result<(), CliError> {
...
fn map_store(error: StoreError) -> CliError {
    match error {
        ...
        StoreError::GroupDomain(multiplex_domain::GroupError::StaleRevision { actual, .. }) => {
            CliError::new(
                ErrorCode::Conflict,
...
pub(crate) fn decode_host_lifecycle(
    value: i32,
) -> Result<multiplex_domain::HostLifecycle, CliError> {
    match wire::Lifecycle::try_from(value).unwrap_or(wire::Lifecycle::Unspecified) {
        wire::Lifecycle::Starting => Ok(multiplex_domain::HostLifecycle::Starting),
        wire::Lifecycle::Ready => Ok(multiplex_domain::HostLifecycle::Ready),
        wire::Lifecycle::Stopping => Ok(multiplex_domain::HostLifecycle::Stopping),
        wire::Lifecycle::Exited => Ok(multiplex_domain::HostLifecycle::Exited),
        wire::Lifecycle::Failed => Ok(multiplex_domain::HostLifecycle::Failed),
...
pub(crate) const fn host_lifecycle_name(
    lifecycle: multiplex_domain::HostLifecycle,
) -> &'static str {
    match lifecycle {
        multiplex_domain::HostLifecycle::Starting => "starting",
        multiplex_domain::HostLifecycle::Ready => "ready",
        multiplex_domain::HostLifecycle::Stopping => "stopping",
        multiplex_domain::HostLifecycle::Exited => "exited",
        multiplex_domain::HostLifecycle::Failed => "failed",
        multiplex_domain::HostLifecycle::Orphaned => "orphaned",
    }
}
