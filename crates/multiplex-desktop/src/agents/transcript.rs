...
mod tests {
    ...
    fn transcript_contracts_are_exact_and_disabled_pending_client_approval() {
        ...
        assert!(
            multiplex_domain::compiled_runtime_descriptors()
                .iter()
                .all(
                    |descriptor| descriptor.version_rules.iter().all(|rule| !rule
                        .capabilities
                        .contains(multiplex_domain::RuntimeCapability::TranscriptExport))
                )
        );
    }
    ...
    fn transcript_reader_preserves_provider_order_and_default_consent() {
        ...
        assert_eq!(
            ...
            Some(multiplex_domain::deterministic_content_hash(
    ...
    fn transcript_reader_skips_oversize_record_without_large_record_allocation() {
        ...
        let request = TranscriptRequest {
            limits: multiplex_domain::TranscriptLimits {
                record_bytes: 128,
                ..multiplex_domain::TranscriptLimits::default()
            },
