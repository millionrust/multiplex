    ...
    pub fn ssh_access_policy(&self) -> SshAccessPolicy {
        match (self.auth_mode, self.certificate_path.is_some()) {
            ...
            (AuthMode::PrivateKey, true) => SshAccessPolicy {
                authentication: multiplex_domain::SshAuthenticationKind::OpenSshCertificate,
                certificate_signer: Some(multiplex_domain::SshCertificateSigner::PrivateKey),
                agent_forwarding: multiplex_domain::SshAgentForwardingPolicy::Disabled,
            },
            (AuthMode::LocalAgent, _) => SshAccessPolicy {
                authentication: multiplex_domain::SshAuthenticationKind::LocalAgent,
        ...
        match self.auth_mode {
            ...
            AuthMode::PrivateKey => {
                ...
                match (policy.authentication, self.certificate_path.clone()) {
                    (
                        multiplex_domain::SshAuthenticationKind::OpenSshCertificate,
...
pub struct SavedAppAttachedSession {
    ...
    #[serde(default)]
    pub activity: multiplex_domain::ActivityAggregate,
    #[serde(default)]
    pub pinned: bool,
    #[serde(default)]
...
pub struct SavedDurableHost {
    ...
    #[serde(default)]
    pub runtime_recognition: Option<multiplex_domain::RuntimeRecognition>,
    #[serde(skip)]
    pub conversation_handle: Option<multiplex_domain::ConversationHandle>,
    #[serde(default)]
    pub executable: Option<String>,
    #[serde(default)]
    pub permission_policy: multiplex_domain::PermissionPolicy,
    #[serde(default)]
    ...
    fn normalize_app_attached_sessions(&mut self) {
        for session in &mut self.app_attached_sessions {
            ...
            session.title = session
                .title
                .chars()
                .take(multiplex_domain::MAX_SESSION_TITLE_SCALARS)
                .collect();
        }
...
impl SavedAppAttachedSession {
    pub fn to_hosted_session(&self) -> Result<HostedSession, multiplex_domain::SessionStateError> {
        ...
        state.upsert_app_attached_session(super::SavedAppAttachedSession {
            ...
            durable_host: Some(super::SavedDurableHost {
                ...
                conversation_handle: Some(
                    multiplex_domain::ConversationHandle::codex(
                        "019cf76d-0493-77d1-8572-3fb4ac801ac8",
                    )
                    .unwrap(),
                ),
                executable: None,
                permission_policy: multiplex_domain::PermissionPolicy::default(),
