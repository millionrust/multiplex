...
impl GitRunner {
    pub fn inspect(
        ...
        fs::create_dir_all(&parent)
            .map_err(|error| filesystem_error(error, WorktreeError::InvalidPath))?;
        let canonical_parent = multiplex_domain::canonical_path(&parent)
            .map_err(|error| filesystem_error(error, WorktreeError::InvalidPath))?;
    ...
    pub fn verify(
        ...
        plan.validate()?;
        cancellation_check(cancellation)?;
        let canonical = multiplex_domain::canonical_path(plan.managed_path.as_path())
...
fn parse_worktree_porcelain_z(bytes: &[u8]) -> Result<Vec<GitWorktreeRecord>, WorktreeError> {
    ...
    if records.len() > multiplex_domain::MAX_WORKTREE_REGISTRATIONS {
        return Err(WorktreeError::ResourceLimit {
            limit: multiplex_domain::MAX_WORKTREE_REGISTRATIONS,
        });
    }
    Ok(records)
}
