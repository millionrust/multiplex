...
impl MultiplexApp {
            ...
            let result = cx
                ...
                .spawn(async move {
                    ...
                    if cancellation.is_cancelled() {
                        return Err(multiplex_client::HostReconciliationError {
                            code: multiplex_client::HostReconciliationErrorCode::Cancelled,
                        });
                    }
    ...
    fn reconcile_visible_session_selection(&mut self, previous: &[AccessibleRowId]) {
        ...
        self.session_sidebar.selected_session =
            multiplex_ui_contract::reconcile_collection_selection(previous, &next, selected)
                .selected
    ...
    pub(super) fn runtime_inspector_semantic_snapshot(
        ...
        if session.route != multiplex_domain::SessionLaunchRoute::DurableHost {
            return None;
        }
        ...
        let confidence = recognition
            ...
            .unwrap_or(multiplex_domain::RecognitionConfidence::Uncertain);
        let confidence_message = match confidence {
            multiplex_domain::RecognitionConfidence::Verified => {
                MessageId::RuntimeConfidenceVerified
            }
            multiplex_domain::RecognitionConfidence::Observed => {
                MessageId::RuntimeConfidenceObserved
            }
            multiplex_domain::RecognitionConfidence::Uncertain => {
        ...
        Some(PresetRuntimeSemanticSnapshot {
            ...
            state: if occupant.is_none() {
                ...
                || confidence != multiplex_domain::RecognitionConfidence::Verified
            {
    ...
    fn product_session_controls(
        ...
        if let Some(record) = record {
            if record.route == multiplex_domain::SessionLaunchRoute::DurableHost {
                controls.push(product_button(
    ...
    fn render_session_row(
        ...
        let recognition = session
            ...
            .as_ref()
            .and_then(|host| host.runtime_recognition.as_ref());
        let runtime = (session.route == multiplex_domain::SessionLaunchRoute::DurableHost)
            .then(|| runtime_inspector_projection(recognition));
        let transcript = (session.route == multiplex_domain::SessionLaunchRoute::DurableHost)
                    ...
                    v_flex()
                        ...
                        .child(
                            h_flex()
                                ...
                                .when(
                                    session.route
                                        == multiplex_domain::SessionLaunchRoute::DurableHost,
                                    |this| {
                                        this.child(
...
fn accessible_group_row_id(id: AccessibleRowId) -> Option<GroupId> {
    (id.kind == multiplex_ui_contract::AccessibleRowKind::Group)
...
fn accessible_session_row_id(id: AccessibleRowId) -> Option<HostedSessionId> {
    (id.kind == multiplex_ui_contract::AccessibleRowKind::Session)
...
fn title_source_label(source: multiplex_domain::TitleSource) -> String {
    match source {
        multiplex_domain::TitleSource::Default => {
            localization::session_library_title_source_default()
        }
        multiplex_domain::TitleSource::Automatic => {
            localization::session_library_title_source_automatic()
        }
        multiplex_domain::TitleSource::Imported => {
            localization::session_library_title_source_imported()
        }
        multiplex_domain::TitleSource::Manual => {
            localization::session_library_title_source_manual()
        }
    }
}

fn activity_label(activity: &multiplex_domain::ActivityAggregate) -> String {
    let state = match activity.state {
        multiplex_domain::ActivityState::Unknown => {
            localization::session_library_activity_unknown()
        }
        multiplex_domain::ActivityState::Idle => localization::session_library_activity_idle(),
        multiplex_domain::ActivityState::Busy => localization::session_library_activity_busy(),
        multiplex_domain::ActivityState::NeedsInput => {
            localization::session_library_activity_needs_input()
        }
        multiplex_domain::ActivityState::Done => localization::session_library_activity_done(),
        multiplex_domain::ActivityState::Failed => localization::session_library_activity_failed(),
    };
    let confidence = if activity.stale {
        return format!("{state} ({})", localization::runtime_stale_label());
    } else if activity.confidence == multiplex_domain::ActivityConfidence::Verified {
...
fn session_ownership_label(route: multiplex_domain::SessionLaunchRoute) -> String {
    match route {
        multiplex_domain::SessionLaunchRoute::LegacyAppAttached => "App-attached".to_string(),
        multiplex_domain::SessionLaunchRoute::DurableHost => "Durable".to_string(),
    }
}
...
fn labeled_status(kind: multiplex_ui_contract::StatusKind, label: String) -> gpui::Div {
    h_flex()
        .items_center()
...
mod tests {
    use super::*;
    use multiplex_domain::{PositionKey, PresetId, SessionLaunchRoute, SessionOrigin};
