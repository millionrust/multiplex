...
use multiplex_domain::NotificationMode;
    ...
    fn setting_presentation(&self, id: SettingId, cx: &App) -> SettingPresentation {
        ...
        let value = match id {
            ...
            SettingId::DevelopmentLocale => Value::Choice(match localization::current_locale() {
                multiplex_ui_contract::Locale::EnUs => MessageId::SettingsLocaleEnglish,
                multiplex_ui_contract::Locale::EnXa => MessageId::SettingsLocaleExpanded,
                multiplex_ui_contract::Locale::ArXb => MessageId::SettingsLocaleRtl,
            }),
        ...
        match command {
            ...
            SettingsAccessibilityCommand::SetSearchValue => {
                if let Some(SemanticActionValue::Text(value)) = value
                    && value.chars().count() <= multiplex_ui_contract::MAX_SETTINGS_QUERY_CHARS
                    && !value.contains('\0')
                {
        ...
        match (id, value) {
            ...
            (id, Some(SemanticActionValue::Text(value)))
                if value.chars().count()
                    <= multiplex_ui_contract::MAX_SEMANTIC_ACTION_VALUE_CHARS
                    && !value.contains('\0') =>
            {
                let input = match id {
