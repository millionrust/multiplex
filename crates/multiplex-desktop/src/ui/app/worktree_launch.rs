...
fn child_project_label(inspection: &WorktreeInspection) -> String {
    ...
    format!("{} · {branch}", inspection.repository_basename)
        .chars()
        .take(multiplex_domain::MAX_LABEL_SCALARS)
        .collect()
}

fn managed_path_preview(plan: &multiplex_domain::WorktreePlan) -> String {
...
mod tests {
    use super::*;
    use multiplex_domain::WorktreePlan;
    ...
    fn child_label_is_bounded_and_uses_safe_basename_only() {
        ...
        let canonical_managed = multiplex_domain::CanonicalPath::resolve(&managed).unwrap();
        let inspection = WorktreeInspection {
            plan: WorktreePlan::new(
                ...
                multiplex_domain::ManagedPath::new(canonical_managed.as_path().join("child"))
                    .unwrap(),
            )
            .unwrap(),
