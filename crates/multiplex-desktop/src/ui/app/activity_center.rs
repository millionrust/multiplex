...
use multiplex_store::{NotificationRepository, NotificationSnapshot, NotificationStoreError};
...
impl ActivityCenterState {
    ...
    fn observe_transition_once(
        ...
        let decision = match reduce_notification(
            ...
            snapshot.policy,
            event,
            if matches!(current.title_source, multiplex_domain::TitleSource::Manual) {
                current.title.as_str()
            } else {
                multiplex_domain::GENERIC_NOTIFICATION_TITLE
            },
            deep_link,
            NotificationContext {
    ...
    ) -> Result<HostedSessionId, multiplex_domain::DeepLinkFailure> {
        let ledger = &self
            .snapshot
            .as_ref()
            .ok_or(multiplex_domain::DeepLinkFailure::Unknown)?
            .ledger;
...
fn activity_status(activity: NotificationActivity) -> multiplex_ui_contract::StatusKind {
    match activity {
        NotificationActivity::NeedsInput => multiplex_ui_contract::StatusKind::Attention,
        NotificationActivity::Done => multiplex_ui_contract::StatusKind::Done,
        NotificationActivity::Failed => multiplex_ui_contract::StatusKind::Error,
    }
}
    ...
    fn open_activity_record(
        ...
        let state = match current.as_ref() {
            ...
            None => DeepLinkSessionState {
                ...
                exists: false,
                archived: false,
                generation: multiplex_domain::OccupantGeneration::ZERO,
                activity: ActivityState::Unknown,
                activity_sequence: multiplex_domain::HostSequence::ZERO,
            },
        };
