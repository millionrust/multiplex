...
impl SessionLibraryState {
    ...
    pub fn confirm_removal(
        ...
        saved: &mut SavedState,
        confirmation: &str,
    ) -> Result<multiplex_store::QuarantinedSession, StoreError> {
        let plan = self
            .pending_removal
            .as_ref()
            .ok_or(StoreError::SessionDomain(SessionStateError::Unavailable))?;
...
mod local_controller_conformance_tests {
    ...
    use multiplex_domain::{PositionKey, SessionLaunchRoute, SessionOrigin, TitleSource};
...
mod activity_tests {
    ...
    fn activity_read_watermark_clears_only_the_sequence_actually_displayed() {
        ...
        multiplex_domain::reduce_session(
            &mut session,
            SessionMutation::ObserveOutput {
        ...
        multiplex_domain::reduce_session(
            ...
            SessionMutation::ApplyActivity {
                activity,
                visible_through: Some(multiplex_domain::ReadWatermark(OutputSequence::new(5))),
            },
        )
        .unwrap();
