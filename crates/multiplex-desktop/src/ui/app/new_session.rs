...
impl MultiplexApp {
    ...
    pub(super) fn cancel_new_session(&mut self, cx: &mut Context<Self>) {
        ...
        if let Some(pane_id) = pane_id {
            ...
            if let Some(id) = hosted_session_id {
                self.mutate_session(
                    id,
                    multiplex_domain::SessionMutation::SetLifecycle(HostedSessionState::Cancelled),
                );
            }
    ...
    fn finish_new_session_validation(
        ...
        let runtime_detection = preset.runtime.as_ref().and_then(|runtime_id| {
            self.preset_library
                ...
                .filter(|entry| {
                    entry.result.status == multiplex_domain::RuntimeDetectionStatus::Available
                        && entry
                            .executable
                            .as_ref()
        ...
        let (title, title_source) = if initial_input.trim().is_empty() {
            ...
            (
                multiplex_domain::automatic_title_from_explicit_input(
                    &initial_input,
                    resolved.session_id,
                )
                .as_str()
        ...
        if let Some(pane) = self.pane_mut(pane_id) {
            pane.app_attached = Some(AppAttachedPaneState {
                ...
                dev_urls: multiplex_client::DevUrlProjection::new(resolved.session_id),
            });
            pane.terminal_focus_mode = multiplex_ui_contract::TerminalFocusMode::Chrome;
    ...
    pub(super) fn stop_app_attached_session(&mut self, pane_id: u64, cx: &mut Context<Self>) {
            ...
            (!pane.closed).then(|| {
                pane.app_attached.as_ref().map(|attached| {
                    (
                        ...
                        attached.route == multiplex_domain::SessionLaunchRoute::DurableHost,
                    )
                })
            })?
        }) else {
        ...
        if !self.mutate_session(
            hosted_session_id,
            multiplex_domain::SessionMutation::SetLifecycle(HostedSessionState::Stopping),
        ) {
            return;
        }
    ...
    pub(super) fn app_attached_ready(&mut self, pane_id: u64, cx: &mut Context<Self>) {
        ...
        self.mutate_session(
            session_id,
            multiplex_domain::SessionMutation::SetLifecycle(
                if self
                    .pane(pane_id)
                    .and_then(|pane| pane.app_attached.as_ref())
                    .is_some_and(|session| {
                        session.route == multiplex_domain::SessionLaunchRoute::DurableHost
                    })
                {
    ...
    pub(super) fn set_app_attached_terminal_state(
        ...
        self.mutate_session(
            session_id,
            multiplex_domain::SessionMutation::SetLifecycle(state),
        );
        let resume_error = if cancelled {
            multiplex_domain::ResumeError::Cancelled
        } else if message.contains("continuity") {
            multiplex_domain::ResumeError::ContinuityConflict
        } else {
            multiplex_domain::ResumeError::ProviderUnavailable
        };
    ...
    std::env::var_os("PATH")
        .map(|path| {
            std::env::split_paths(&path)
                .take(multiplex_domain::MAX_PATH_SEARCH_DIRECTORIES)
                .collect()
        })
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn path_snapshot_is_bounded() {
        assert!(explicit_path_snapshot().len() <= multiplex_domain::MAX_PATH_SEARCH_DIRECTORIES);
    }
