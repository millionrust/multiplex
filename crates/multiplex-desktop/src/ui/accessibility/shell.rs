...
#[cfg(target_os = "macos")]
use multiplex_accessibility_macos::MacAccessibilityBridge;
#[cfg(target_os = "macos")]
use std::sync::Arc;
#[cfg(target_os = "macos")]
...
impl ShellAccessibilityAdapter {
    ...
    pub fn sync(&mut self, mut snapshot: ShellSemanticSnapshot) {
        ...
        let result = (|| {
            ...
            self.last_product_dialog_open = product_dialog_open;
            Ok::<_, multiplex_ui_contract::SemanticError>(())
        })();
...
fn semantic_shape(snapshot: &ShellSemanticSnapshot) -> u64 {
    ...
    if let Some(terminal) = snapshot.terminal.as_ref() {
        ...
        matches!(
            ...
            multiplex_ui_contract::TerminalLifecycle::Gap
                | multiplex_ui_contract::TerminalLifecycle::Offline
                | multiplex_ui_contract::TerminalLifecycle::Backpressured
                | multiplex_ui_contract::TerminalLifecycle::Error
                | multiplex_ui_contract::TerminalLifecycle::PermissionDenied
        )
        .hash(&mut hasher);
        match terminal.focus_mode {
            multiplex_ui_contract::TerminalFocusMode::Chrome => 0_u8,
            multiplex_ui_contract::TerminalFocusMode::Input => 1,
            multiplex_ui_contract::TerminalFocusMode::AccessibleReview => 2,
        }
        .hash(&mut hasher);
        ...
        terminal
            ...
            .is_some_and(|announcement| {
                matches!(
                    announcement,
                    multiplex_ui_contract::TerminalAnnouncement::Attention
                        | multiplex_ui_contract::TerminalAnnouncement::Gap
                )
            })
            .hash(&mut hasher);
        if terminal.focus_mode == multiplex_ui_contract::TerminalFocusMode::AccessibleReview
            && !terminal.recording_friendly
        {
            multiplex_ui_contract::terminal_semantic_chunk_count(&terminal.terminal.text)
                .hash(&mut hasher);
        }
    }
    hasher.finish()
