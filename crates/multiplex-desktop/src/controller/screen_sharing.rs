...
use multiplex_domain::AuthenticatedPeer;
use multiplex_screen_capture::{CaptureConfig, Damage, FrameSource};
use multiplex_screen_codec::{Frame, Size};
use multiplex_screen_host::{ScreenHost, ScreenHostEvent, ScreenHostHandle};
use multiplex_screen_input::{DisplayLayout, DisplayPlacement, Injector};
use multiplex_screen_protocol::{ControlHolder, SurfaceInfo};
use multiplex_screen_session::{HostConfig, InputEvent};
...
pub struct ScreenWatcher {
    pub device_id: multiplex_domain::ControllerDeviceId,
...
impl ScreenSharing {
    ...
    fn remove_watcher(&self, device_id: multiplex_domain::ControllerDeviceId) {
        self.watchers
            .lock()
            .expect("watchers")
    ...
    fn set_controlling(&self, device_id: multiplex_domain::ControllerDeviceId, controlling: bool) {
...
#[cfg(target_os = "macos")]
fn displays() -> Result<Vec<SharedDisplay>, multiplex_screen_capture::CaptureError> {
    // Capture at the display's own scale; the viewer downscales for its viewport.
    const SCALE: f32 = 2.0;
    Ok(multiplex_screen_capture::displays()?
        .into_iter()
        .enumerate()
...
fn spawn_capture(display: SharedDisplay, handle: ScreenHostHandle) {
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            #[cfg(target_os = "macos")]
            let started = multiplex_screen_capture::ScreenCaptureKitSource::start(config);
            #[cfg(target_os = "windows")]
            let started = multiplex_screen_capture::DesktopDuplicationSource::start(config);
    ...
    fn open_portal(&self, restore_token: Option<String>) {
        ...
        let _ = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let Ok(source) = multiplex_screen_capture::PortalScreenCastSource::start(
                    config,
...
fn open_injector(
    layout: DisplayLayout,
) -> Option<Injector<multiplex_screen_input::CoreGraphicsSink>> {
    multiplex_screen_input::CoreGraphicsSink::new()
        .ok()
...
mod tests {
    ...
    fn one_granted_stream_feeds_every_watcher_and_forgets_the_ones_that_left() {
        ...
        let frames = (0..4).map(|index| {
            multiplex_screen_capture::CapturedFrame::tight(
                size,
        ...
        fan_out(
            multiplex_screen_capture::ReplaySource::new(frames),
            1,
            &watchers,
            &enabled,
        );
