...
use multiplex_store::{AtomicWriter as _, SystemAtomicWriter};
...
fn run_foreground(installed_path: PathBuf) -> Result<(), RelayHostServiceError> {
    ...
    runtime.block_on(async move {
        ...
        let controller_root = crate::storage::controller_store_dir()
            .map_err(|_| RelayHostServiceError("relay.host.storage_unavailable"))?;
        let repository = multiplex_store::ControllerDeviceRepository::open(controller_root.clone())
...
mod tests {
    use super::*;
    use multiplex_relay_client::MemoryRelaySecretStore;
