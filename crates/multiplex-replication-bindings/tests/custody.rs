...
impl CanonicalizeForTest for Path {
    fn canonicalize_for_test(&self) -> PathBuf {
        multiplex_domain::canonical_path(self).unwrap()
    }
}
...
fn mobile_enrollment_review_and_accept_preserve_exact_request_and_custody() {
    use multiplex_store::replication::{ReplicationEnrollmentRequest, ReplicationProductService};
    ...
    *store.error.lock().unwrap() = None;
    ...
    let key = multiplex_domain::ReplicationRecordKey::new(
        multiplex_domain::ReplicationCollectionId::new("desktop-profiles").unwrap(),
        multiplex_domain::ReplicationRecordId::new("fixture-host").unwrap(),
    );
    desktop
    ...
    let host_key = multiplex_domain::ReplicationRecordKey::new(
        multiplex_domain::ReplicationCollectionId::new("desktop-profiles").unwrap(),
        multiplex_domain::ReplicationRecordId::new(host_id).unwrap(),
    );
...
fn host_discovery_is_authenticated_inert_and_reopens_after_import() {
    ...
    tampered
        ...
        .unwrap()
        .candidates[0]
        .author = multiplex_domain::ReplicationReplicaId::new("wrong-author").unwrap();
    assert!(
        facade
    ...
    wrong_workspace.workspace_id =
        multiplex_domain::ReplicationWorkspaceId::new("other-workspace").unwrap();
    assert!(
        facade
