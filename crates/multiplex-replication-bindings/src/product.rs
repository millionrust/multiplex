...
use multiplex_store::replication::{
...
impl MobileReplicationProduct {
    ...
    pub fn cancel_pending_enrollment(
        ...
        let _lock = self.lock()?;
        let expected =
            multiplex_store::replication::ReplicationEnrollmentRequest::from_canonical_bytes(
                &expected_request,
            )
            .map_err(map_error)?;
    ...
    pub fn accept_enrollment(
        ...
        let bundle =
            multiplex_store::replication::ReplicationEnrollmentBundle::from_canonical_bytes(
                &bundle_bytes,
            )
            .map_err(map_error)?;
    ...
    pub fn review_host_transfer(
        ...
        let review = service
            .review_record_transfer(&document_bytes, &key)
            .map_err(map_error)?;
        let host = multiplex_protocol::review_replicated_host(
            review.value(),
            "desktop-profiles",
            &record_id,
        )
    ...
    pub fn apply_host_transfer(
        ...
        let review = service
            .review_record_transfer(&document_bytes, &key)
            .map_err(map_error)?;
        multiplex_protocol::review_replicated_host(review.value(), "desktop-profiles", &record_id)
            .map_err(|_| MobileReplicationError::Invalid)?;
...
fn host_collection() -> Result<multiplex_domain::ReplicationCollectionId, MobileReplicationError> {
    multiplex_domain::ReplicationCollectionId::new("desktop-profiles")
        .map_err(|_| MobileReplicationError::Invalid)
}

fn project_hosts(
    records: Vec<multiplex_store::replication::ReplicationProductRecord>,
) -> Result<Vec<MobileReplicatedHost>, MobileReplicationError> {
    ...
    for record in records {
        ...
        let host = multiplex_protocol::review_replicated_host(value, "desktop-profiles", id)
            .map_err(|_| MobileReplicationError::Invalid)?;
...
fn record_key(
    collection: String,
    record_id: String,
) -> Result<multiplex_domain::ReplicationRecordKey, MobileReplicationError> {
    Ok(multiplex_domain::ReplicationRecordKey::new(
        multiplex_domain::ReplicationCollectionId::new(collection)
            .map_err(|_| MobileReplicationError::Invalid)?,
        multiplex_domain::ReplicationRecordId::new(record_id)
            .map_err(|_| MobileReplicationError::Invalid)?,
    ))
}
    ...
    match error {
        ...
        | ReplicationProductError::Store(
            ...
            | ReplicationStoreError::Domain(
                multiplex_domain::ReplicationError::MalformedDocument
                | multiplex_domain::ReplicationError::DocumentTooLarge
                | multiplex_domain::ReplicationError::UnsupportedSchema
                | multiplex_domain::ReplicationError::WorkspaceMismatch,
            )
