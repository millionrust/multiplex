...
impl<V: TicketVerifier> HostSession<V> {
    ...
    fn open(
        ...
        if let Some(request) = hello.resume {
            ...
            {
                if encoder.resume(request.sequence) == multiplex_screen_codec::Resume::Partial {
        ...
        match message {
            ...
            Message::CacheMiss {
                ...
                if let Some(subscription) = self.subscriptions.get_mut(&surface) {
                    subscription
                        .encoder
                        .cache_miss(multiplex_screen_codec::CacheMiss { tile, hash });
                }
                Ok(Vec::new())
            }
    ...
    /// [`multiplex_screen_codec::Encoder::motion_region_bytes_per_second`].
